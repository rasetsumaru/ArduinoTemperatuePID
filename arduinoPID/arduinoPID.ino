...
void loop(void) {
  ...
  if (Serial.available() > 0 && mode == 0){
    ...
    if (serialmessage.toInt() > 0 && serialmessage.toInt() < 3){
      ...
      Serial.print(F("Digite um valor para entre 20.00 e 50.00 para configurar o setpoint."));
      Serial.print('\n');
    }
    else{
      Serial.print('\n');
  ...
  if (Serial.available() > 0 && mode != 0 && setpoint == 0.0){
    ...
    if (setpointmessage >= 20.0 && setpointmessage <= 50.0){
      ...
      Serial.println(F("Por favor, digite um valor válido."));
      Serial.println(F("Digite um valor para entre 20.00 e 50.00 para configurar o setpoint."));
